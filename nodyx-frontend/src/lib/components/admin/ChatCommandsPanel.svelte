<script lang="ts">
	...
	const INSERTABLE_VARS = [
		{ token: '{nodyx_url}', labelKey: 'timer.var_nodyx_label' },
		{ token: '{streamer}',  labelKey: 'timer.var_streamer_label' },
		{ token: '{uptime}',    labelKey: 'timer.var_uptime_label' },
	]
	...
	const PRESETS: CmdPreset[] = [
		{
			...
			hintKey: 'cmd.preset_discord_hint',
			fill: () => {
				formName     = '!discord'
				formTemplate = tFn('cmd.preset_discord_tpl')
				formModOnly  = false
				formCooldown = 30
				formEnabled  = true
			},
		},
		{
			...
			hintKey: 'cmd.preset_schedule_hint',
			fill: () => {
				formName     = '!schedule'
				formTemplate = tFn('cmd.preset_schedule_tpl')
				formModOnly  = false
				formCooldown = 30
				formEnabled  = true
			},
		},
		{
			...
			hintKey: 'cmd.preset_social_hint',
			fill: () => {
				formName     = '!social'
				formTemplate = tFn('cmd.preset_social_tpl')
				formModOnly  = false
				formCooldown = 60
				formEnabled  = true
			},
		},
		{
			...
			hintKey: 'cmd.preset_lurk_hint',
			fill: () => {
				formName     = '!lurk'
				formTemplate = tFn('cmd.preset_lurk_tpl')
				formModOnly  = false
				formCooldown = 15
				formEnabled  = true
			},
		},
		{
			...
			hintKey: 'cmd.preset_tipeee_hint',
			fill: () => {
				formName     = '!soutien'
				formTemplate = tFn('cmd.preset_tipeee_tpl')
				formModOnly  = false
				formCooldown = 60
				formEnabled  = true
			},
		},
		{
			...
			hintKey: 'cmd.preset_projet_hint',
			fill: () => {
				formName     = '!projet'
				formTemplate = tFn('cmd.preset_projet_tpl')
				formModOnly  = false
				formCooldown = 60
				formEnabled  = true
			},
	...
	function applyPreset(p: CmdPreset): void {
		editingId = null
		p.fill()
		flash(tFn('timer.preset_loaded', { label: p.name }), true)
	}
	...
	async function submitForm(): Promise<void> {
		...
		try {
			...
			if (res.ok) {
				flash(editingId ? tFn('cmd.updated') : tFn('cmd.created'), true)
				resetForm()
				await loadAll()
			} else {
				const data = await res.json().catch(() => ({})) as { error?: string }
				const msg =
					data.error === 'name_reserved_hardcoded' ? tFn('cmd.err_reserved') :
					data.error === 'name_already_used'       ? tFn('cmd.err_used') :
					data.error === 'invalid_name'            ? tFn('cmd.err_invalid_name') :
					data.error ?? tFn('cmd.err_unknown')
				flash(tFn('timer.err_generic', { error: msg }), false)
			}
		} finally {
			formBusy = false
		}
	...
	async function toggleEnabled(c: CustomCommand): Promise<void> {
		...
		if (res.ok) { c.enabled = !c.enabled; flash(c.enabled ? tFn('cmd.toggled_on', { name: c.name }) : tFn('cmd.toggled_off', { name: c.name }), true) }
		else flash(tFn('timer.err_toggle'), false)
	}

	async function removeCmd(c: CustomCommand): Promise<void> {
		if (!confirm(tFn('cmd.delete_confirm', { name: c.name }))) return
		const res = await apiFetch(fetch, `/streamer/chat-commands/${c.id}`, {
		...
		if (res.ok) {
			...
			flash(tFn('cmd.deleted', { name: c.name }), true)
		} else flash(tFn('timer.err_delete'), false)
	}
...
<section class="space-y-6">
	...
	<header class="flex items-start justify-between gap-4 flex-wrap">
		<div>
			<div class="flex items-center gap-2">
				<h2 class="text-lg font-semibold text-zinc-100">{tFn('cmd.title')}</h2>
				<Tooltip text={tFn('cmd.title_tooltip')} position="bottom"/>
			</div>
		...
		<div>
			<div class="flex items-baseline justify-between mb-2">
				<div class="flex items-center gap-1.5">
					<h3 class="text-xs uppercase tracking-wide font-medium text-zinc-500">{tFn('cmd.native_title')}</h3>
					<Tooltip text={tFn('cmd.native_tooltip')}/>
				</div>
	...
	<div>
		<div class="flex items-baseline justify-between mb-2">
			<h3 class="text-xs uppercase tracking-wide font-medium text-zinc-500">{tFn('cmd.configured_title')}</h3>
		...
		{:else}
			...
			<div class="border border-zinc-800 bg-zinc-900">
				<div class="grid grid-cols-[160px_180px_1fr_auto] gap-4 px-4 py-2 border-b border-zinc-800 bg-zinc-950 text-[11px] uppercase tracking-wide font-medium text-zinc-500">
					...
					<span>{tFn('cmd.col_command')}</span>
					<span>{tFn('cmd.col_response')}</span>
						...
						<li class="grid grid-cols-[160px_180px_1fr_auto] gap-4 px-4 py-3 items-start {editingId === c.id ? 'bg-purple-500/[0.04] border-l-2 border-l-purple-500' : ''}">
							...
							<div class="flex flex-col gap-1 text-xs">
								<span class="inline-flex items-center gap-1.5">
									...
									<span class="{c.enabled ? 'text-emerald-300' : 'text-zinc-500'} font-medium">{c.enabled ? tFn('timer.active') : tFn('timer.inactive')}</span>
								</span>
								{#if c.modOnly}
							...
							<div class="flex items-center gap-1.5 justify-end">
								<button type="button" onclick={() => toggleEnabled(c)}
									...
									{c.enabled ? tFn('timer.disable') : tFn('timer.enable')}
								</button>
	...
	<div class="border border-zinc-800 bg-zinc-900">
		<header class="px-4 py-3 border-b border-zinc-800 flex items-center justify-between gap-3">
			<h3 class="text-sm font-semibold text-zinc-100">
				{editingId ? tFn('cmd.edit_title') : tFn('cmd.new_title')}
			</h3>
			{#if editingId}
		...
		<div class="p-4 space-y-5">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					<div class="flex items-center gap-1.5">
						<label class="text-[11px] uppercase tracking-wide font-medium text-zinc-500">{tFn('cmd.name_label')}</label>
						<Tooltip text={tFn('cmd.name_tooltip')}/>
					</div>
					<input type="text" bind:value={formName} maxlength="31" placeholder={tFn('cmd.name_ph')}
				...
				<div>
					<div class="flex items-center gap-1.5">
						<label class="text-[11px] uppercase tracking-wide font-medium text-zinc-500">{tFn('cmd.cooldown_label')}</label>
						<Tooltip text={tFn('cmd.cooldown_tooltip')}/>
					</div>
			...
			<div>
				<div class="flex items-center justify-between gap-2 flex-wrap mb-1.5">
					<div class="flex items-center gap-1.5">
						<label class="text-[11px] uppercase tracking-wide font-medium text-zinc-500">{tFn('cmd.response_label')}</label>
						<Tooltip text={tFn('cmd.response_tooltip')}/>
					</div>
				...
				<textarea bind:this={templateInputEl} bind:value={formTemplate} maxlength="500" rows="3"
					placeholder={tFn('cmd.response_ph')}
			...
			<div class="flex items-center gap-6 flex-wrap">
				<label class="flex items-center gap-2 cursor-pointer">
					...
					<Tooltip text={tFn('cmd.modonly_tooltip')}/>
				</label>
				<label class="flex items-center gap-2 cursor-pointer">
					<input type="checkbox" bind:checked={formEnabled} class="w-4 h-4 accent-purple-500"/>
					<span class="text-sm text-zinc-300">{tFn('cmd.enabled_on_create')}</span>
				</label>
			</div>
		</div>
		...
		<footer class="px-4 py-3 border-t border-zinc-800 bg-zinc-900/60 flex items-center gap-2">
			<button type="button" onclick={submitForm} disabled={formBusy || !formName.trim() || !formTemplate.trim()}
				...
				{editingId ? tFn('timer.save_edit') : tFn('cmd.create_btn')}
			</button>
			{#if editingId}
