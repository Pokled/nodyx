...
<div class="relative w-full h-32 bg-gradient-to-br from-gray-950 via-indigo-950/30 to-gray-950">
	...
	<div class="absolute inset-0 overflow-hidden">
		{#if bannerPreview}
			<img src={bannerPreview} alt={tFn('pedit.banner')} class="w-full h-full object-cover" />
		{/if}
...
<div class="max-w-5xl mx-auto px-6 mt-14 mb-6">
	<h1 class="text-xl font-bold text-white">{tFn('pedit.title')}</h1>
...
>
	...
	<div class="grid grid-cols-5 gap-4">
		...
		<div class="col-span-3 bg-gray-900/80 border border-gray-800 rounded-xl p-5 space-y-4">
			...
			<div>
				<label for="display_name" class="block text-xs text-gray-400 mb-1.5 font-medium">{tFn('pedit.display_name')}</label>
				<input id="display_name" name="display_name" type="text" maxlength="100"
					bind:value={displayName}
					placeholder={tFn('pedit.display_name_ph')}
			...
			<div>
				<label for="name_color" class="block text-xs text-gray-400 mb-1.5 font-medium">
					{tFn('pedit.name_color')} <span class="text-gray-600 font-normal">{tFn('pedit.name_color_sub')}</span>
				</label>
					...
					<div class="flex-1">
						<p class="text-sm font-semibold truncate drop-shadow"
						   style="color: {nameColor}">
							{displayName || profile.username || tFn('pedit.name_preview')}
						</p>
			...
			<div class="grid grid-cols-2 gap-3">
				<div>
					...
					<input id="status" name="status" type="text" maxlength="100"
						placeholder={tFn('pedit.status_ph')}
						value={profile.status ?? ''}
						class="w-full rounded-lg bg-gray-800/70 border border-gray-700 px-3 py-2.5 text-sm text-white
						       placeholder-gray-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 transition-colors" />
				</div>
				<div>
					<label for="location" class="block text-xs text-gray-400 mb-1.5 font-medium">{tFn('pedit.location')}</label>
					<input id="location" name="location" type="text" maxlength="100"
						placeholder={tFn('pedit.location_ph')}
			...
			<div>
				<label for="bio" class="block text-xs text-gray-400 mb-1.5 font-medium">
					{tFn('pedit.bio')} <span class="text-gray-600 font-normal">{tFn('pedit.bio_max')}</span>
				</label>
				<textarea id="bio" name="bio" maxlength="2000" rows="5"
					placeholder={tFn('pedit.bio_ph')}
					class="w-full rounded-lg bg-gray-800/70 border border-gray-700 px-3 py-2.5 text-sm text-white
		...
		<div class="col-span-2 bg-gray-900/80 border border-gray-800 rounded-xl p-5 space-y-5">
				...
				{:else}
					<label class="flex items-center gap-2 cursor-pointer">
						<span class="px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 text-xs text-gray-200 transition-colors">
							{avatarUploading ? tFn('pedit.uploading') : tFn('pedit.choose_file')}
						</span>
						<span class="text-[11px] text-gray-600">{tFn('pedit.avatar_formats')}</span>
			...
			<div>
				<div class="flex items-center justify-between mb-2">
					<p class="text-xs text-gray-400 font-medium">{tFn('pedit.banner')}</p>
					{#if bannerPreview}
				...
				{#if bannerPreview}
					<img src={bannerPreview} alt={tFn('pedit.banner_preview_alt')} class="w-full h-16 object-cover rounded-lg mb-2 border border-gray-700" />
				{:else}
				...
				{:else}
					<label class="flex items-center gap-2 cursor-pointer">
						<span class="px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 text-xs text-gray-200 transition-colors">
							{bannerUploading ? tFn('pedit.uploading') : tFn('pedit.choose_file')}
						</span>
						<span class="text-[11px] text-gray-600">{tFn('pedit.size_5mb')}</span>
						<input type="file" accept="image/jpeg,image/png,image/webp,image/gif"
	...
	<div class="bg-gray-900/80 border border-gray-800 rounded-xl p-5 space-y-5">
		<h2 class="text-xs uppercase tracking-widest text-gray-500 font-semibold flex items-center gap-2">
			...
			{tFn('pedit.section_theme')}
		</h2>

		<div class="grid lg:grid-cols-3 gap-6">
			<!-- Preset grid -->
			<div class="lg:col-span-2 space-y-3">
				<p class="text-xs text-gray-400 font-medium">{tFn('pedit.choose_preset')}</p>
			...
			<div class="space-y-3">
				...
				<p class="text-[11px] text-gray-600">{tFn('pedit.theme_hint')}</p>
			</div>
		</div>

		<!-- Live preview strip -->
		<div>
			<p class="text-xs text-gray-400 font-medium mb-2">{tFn('common.preview')}</p>
	...
	<div class="bg-gray-900/80 border border-gray-800 rounded-xl p-5 space-y-6">
		...
		<div class="grid lg:grid-cols-3 gap-6">
			...
			<!-- Police -->
			<div class="space-y-3">
				<p class="text-xs text-gray-400 font-medium">{tFn('pedit.font')}</p>
				<div class="grid grid-cols-3 gap-1.5">
					{#each FONT_PRESETS as preset}
						...
						>
							{preset.family ? preset.preview : tFn('pedit.default')}
						</button>
					{/each}
				</div>
				<!-- Custom font upload -->
				<div class="pt-1">
					<label class="flex items-center gap-2 cursor-pointer">
						<span class="px-3 py-1.5 rounded-lg bg-gray-700 hover:bg-gray-600 text-xs text-gray-200 transition-colors shrink-0">
							{fontUploading ? tFn('pedit.uploading') : tFn('pedit.import_font')}
						</span>
					...
					{#if nameFontUrl}
						<p class="text-[10px] text-green-500 mt-1">{tFn('pedit.font_loaded')} {nameFontFamily}</p>
					{/if}
			...
			<!-- Halo lumineux -->
			<div class="space-y-3">
				<p class="text-xs text-gray-400 font-medium">{tFn('pedit.glow')}</p>
				<label class="flex items-center gap-2 cursor-pointer select-none">
					...
					<span class="text-xs text-gray-300">{tFn('pedit.enable_glow')}</span>
				</label>
				{#if nameGlowEnabled}
					<div class="space-y-2">
						<div class="flex items-center gap-3">
			...
			<!-- Animations -->
			<div class="space-y-3">
				<p class="text-xs text-gray-400 font-medium">{tFn('pedit.animation')}</p>
				...
				<p class="text-[10px] text-gray-600">{tFn('pedit.anim_hint')}</p>
			</div>
		</div>
	</div>
		...
		<div class="grid grid-cols-2 gap-4">
			<div>
				...
				<div class="flex items-center rounded-lg bg-gray-800/70 border border-gray-700 overflow-hidden focus-within:border-indigo-500 transition-colors">
					...
					<input id="github_username" name="github_username" type="text" maxlength="39"
						placeholder={tFn('pedit.username_ph')} pattern="[a-zA-Z0-9-]+"
			...
			<div>
				<label for="twitter_username" class="block text-xs text-gray-400 mb-1.5 font-medium">
					Twitter / X <span class="text-gray-600 font-normal">{tFn('pedit.eg_handle')}</span>
				</label>
				<div class="flex items-center rounded-lg bg-gray-800/70 border border-gray-700 overflow-hidden focus-within:border-indigo-500 transition-colors">
					...
					<input id="twitter_username" name="twitter_username" type="text" maxlength="100"
						placeholder={tFn('pedit.handle_ph')}
			...
			<div>
				<label for="instagram_username" class="block text-xs text-gray-400 mb-1.5 font-medium">
					Instagram <span class="text-gray-600 font-normal">{tFn('pedit.eg_handle')}</span>
				</label>
				<div class="flex items-center rounded-lg bg-gray-800/70 border border-gray-700 overflow-hidden focus-within:border-indigo-500 transition-colors">
					<span class="px-3 py-2.5 text-gray-500 text-sm border-r border-gray-700 bg-gray-800/50 shrink-0">@</span>
			...
			<div>
				<label for="youtube_channel" class="block text-xs text-gray-400 mb-1.5 font-medium">
					YouTube <span class="text-gray-600 font-normal">{tFn('pedit.eg_youtube')}</span>
				</label>
				<div class="flex items-center rounded-lg bg-gray-800/70 border border-gray-700 overflow-hidden focus-within:border-indigo-500 transition-colors">
					...
					<input id="youtube_channel" name="youtube_channel" type="text" maxlength="200"
						placeholder={tFn('pedit.youtube_ph')}
			...
			<div class="col-span-2">
				<label for="website_url" class="block text-xs text-gray-400 mb-1.5 font-medium">
					{tFn('pedit.website')} <span class="text-gray-600 font-normal">{tFn('pedit.full_url')}</span>
				</label>
				<input id="website_url" name="website_url" type="url" maxlength="500"
					placeholder={tFn('pedit.website_ph')}
	...
	<div class="grid grid-cols-2 gap-4">
		...
		<div class="bg-gray-900/80 border border-gray-800 rounded-xl p-5">
			...
			<label for="tags" class="block text-xs text-gray-400 mb-1.5 font-medium">
				{tFn('pedit.comma_separated')} <span class="text-gray-600">(max 10)</span>
			</label>
			<input id="tags" name="tags" type="text"
				placeholder={tFn('pedit.tags_ph')}
				value={profile.tags?.join(', ') ?? ''}
				class="w-full rounded-lg bg-gray-800/70 border border-gray-700 px-3 py-2.5 text-sm text-white
				       placeholder-gray-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 transition-colors" />
			<p class="text-[11px] text-gray-600 mt-2">{tFn('pedit.interests_hint')}</p>
		</div>
		...
		<div class="bg-gray-900/80 border border-gray-800 rounded-xl p-5">
			...
			<div class="space-y-2 mb-3">
				...
				{/each}
				{#if links.length === 0}
					<p class="text-xs text-gray-600 py-2">{tFn('pedit.no_links')}</p>
				{/if}
			</div>
