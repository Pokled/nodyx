<script lang="ts">
    ...
    let logs = $state<string[]>([tFn('astatus.log_ready'), tFn('astatus.log_waiting')]);
    ...
    async function startTest() {
        ...
        addLog(tFn('astatus.log_udp'));
        await new Promise(r => setTimeout(r, 800));
        visualChecks.UDP = diag.udp ? 'OK' : tFn('astatus.st_error');

        visualChecks.P2P = 'SCANNING';
        addLog(tFn('astatus.log_p2p'));
        await fillBar('p2p', 6);
        ...
        if (diag.p2p) {
            ...
            p2pSegments = 2;
            visualChecks.P2P = tFn('astatus.st_fail');
        }

        visualChecks.RELAY = 'SYNCING';
        addLog(tFn('astatus.log_relay'));
...
<svelte:head><title>{tFn('astatus.page_title')}</title></svelte:head>
...
<div class="space-y-6 font-sans">
    <div class="flex items-center justify-between">
        ...
        >
            ...
            {isTesting ? tFn('astatus.analyzing') : tFn('astatus.run_diag')}
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
            <div class="rounded-xl border border-gray-800 bg-gray-900/50 p-5">
                <h2 class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-4 italic">{tFn('astatus.modules_check')}</h2>
            ...
            <div class="rounded-xl border border-gray-800 bg-black/40 p-5 h-[168px] flex flex-col">
                <h2 class="text-xs font-bold text-gray-600 uppercase mb-3">{tFn('astatus.connection_logs')}</h2>
        ...
        <div class="rounded-xl border border-gray-800 bg-gray-900/50 p-6 flex flex-col justify-between">
            <h2 class="text-xs font-bold text-gray-500 uppercase tracking-widest mb-6 italic">{tFn('astatus.sovereignty_index')}</h2>
            
            <div class="space-y-10">
                <div>
                    <div class="flex justify-between text-[11px] font-bold text-gray-400 mb-3 uppercase">
                        <span>{tFn('astatus.direct_link')}</span>
                ...
                <div>
                    <div class="flex justify-between text-[11px] font-bold text-gray-400 mb-3 uppercase">
                        <span>{tFn('astatus.relay_flow')}</span>
