<script lang="ts">
	...
	const ACTION_META: Record<string, { label: string; color: string; icon: string }> = {
		ban_user:             { label: 'alog.act_ban',           color: 'text-red-400    bg-red-900/40    border-red-800/50',    icon: '🔨' },
		unban_user:           { label: 'alog.act_unban',         color: 'text-green-400  bg-green-900/40  border-green-800/50',  icon: '✅' },
		kick_member:          { label: 'alog.act_kick',          color: 'text-orange-400 bg-orange-900/40 border-orange-800/50', icon: '👢' },
		change_role:          { label: 'alog.act_role',   color: 'text-indigo-400 bg-indigo-900/40 border-indigo-800/50', icon: '🏅' },
		delete_thread:        { label: 'alog.act_delete_thread',  color: 'text-red-400    bg-red-900/40    border-red-800/50',    icon: '🗑️' },
		pin_thread:           { label: 'alog.act_pin',       color: 'text-yellow-400 bg-yellow-900/40 border-yellow-800/50', icon: '📌' },
		unpin_thread:         { label: 'alog.act_unpin',    color: 'text-gray-400   bg-gray-800      border-gray-700',      icon: '📌' },
		lock_thread:          { label: 'alog.act_lock',    color: 'text-red-400    bg-red-900/40    border-red-800/50',    icon: '🔒' },
		unlock_thread:        { label: 'alog.act_unlock',  color: 'text-green-400  bg-green-900/40  border-green-800/50',  icon: '🔓' },
		create_announcement:  { label: 'alog.act_create_ann', color: 'text-sky-400    bg-sky-900/40    border-sky-800/50',    icon: '📢' },
		delete_announcement:  { label: 'alog.act_delete_ann', color: 'text-gray-400 bg-gray-800  border-gray-700',        icon: '📢' },
	}
	...
	function timeAgo(d: string) {
		...
		if (m < 1)   return tFn('alog.just_now')
		if (m < 60)  return tFn('alog.ago_min', { n: m })
		if (h < 24)  return tFn('alog.ago_h', { n: h })
		if (day < 7) return tFn('alog.ago_j', { n: day })
	...
	function metaDesc(entry: any): string {
		...
		if (entry.action === 'ban_user') {
			const parts = []
			if (m.reason) parts.push(tFn('alog.reason', { reason: m.reason }))
			if (m.ban_ip) parts.push(tFn('alog.ip_banned'))
			if (m.ban_email) parts.push(tFn('alog.email_banned'))
			return parts.join(' · ') || ''
		}
...
<svelte:head><title>{tFn('alog.page_title')}</title></svelte:head>

<div class="space-y-5">
	<div class="flex items-start justify-between gap-4 flex-wrap">
		<div>
			<h1 class="text-2xl font-bold text-white">{tFn('alog.title')}</h1>
			<p class="text-sm text-gray-500 mt-0.5">{data.total > 1 ? tFn('alog.total_many', { n: data.total }) : tFn('alog.total_one', { n: data.total })}</p>
		</div>
		...
		<form method="GET" class="flex flex-wrap gap-2">
			<select name="action" onchange={(e) => (e.currentTarget as HTMLSelectElement).form?.submit()}
				...
				<option value="">{tFn('alog.all_actions')}</option>
				{#each ACTIONS as a}
					<option value={a} selected={a === data.filterAction}>{ACTION_META[a] ? tFn(ACTION_META[a].label) : a}</option>
				{/each}
			</select>
			<input
				type="text"
				name="actor"
				placeholder={tFn('alog.filter_ph')}
			...
			<table class="w-full text-sm">
				<thead class="bg-gray-900 border-b border-gray-800 text-xs text-gray-500 uppercase tracking-wider">
					<tr>
						...
						<th class="px-4 py-3 text-left w-32">{tFn('alog.col_admin')}</th>
						<th class="px-4 py-3 text-left w-36">{tFn('alog.col_action')}</th>
						<th class="px-4 py-3 text-left">{tFn('alog.col_target')}</th>
						<th class="px-4 py-3 text-left">{tFn('alog.col_detail')}</th>
					</tr>
				</thead>
	...
	{#if data.total > data.limit}
		<div class="flex items-center justify-between">
			<span class="text-sm text-gray-500">
				{tFn('alog.pagination', { from: data.offset + 1, to: Math.min(data.offset + data.limit, data.total), total: data.total })}
			</span>
