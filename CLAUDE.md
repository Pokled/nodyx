...
- **i18n ships WITH the feature, never after it.** Any new user-facing string is a translation key from its first commit: `tFn('key')`, added to **both** `fr.json` and `en.json` in the same pull request. Never a hardcoded string "to be extracted later", never a French-only key. Four CI gates enforce this and will fail the build: no hardcoded strings (`i18n:check`), no key referenced but missing (`i18n:keys:check`), English at full parity with the source (`i18n:parity:check`), and no placeholder corrupted by a translation (`i18n:placeholders:check`). Run them locally before pushing. The 2026-07/08 extraction marathon (about 100 pull requests to retrofit i18n onto existing pages) is exactly what these gates exist to prevent happening twice.
